...
suite const_noexcept_qualified = []
{
    ...
    feature("call from const noexcept member function") = []
    {
        ...
        given("a non-const object with qualified member functions") = []
        {
            ...
            {
                expect(call({fn<&A_nice::h>, w}) == ch<'h'>);
                expect(call({fn<&A_nice::h>, &w}) == ch<'h'>);
            };

            then("can call const member function on const lvalue") = [&]
            { expect(call({fn<&A_nice::h>, std::as_const(w)}) == ch<'h'>); };
        };
    };
};
...
static_assert(not std::is_constructible_v<U, fn_t<&A::data>, A>,
              "cannot bind rvalue");

static_assert(not std::is_constructible_v<U, fn_t<&A::g>, A &>,
              "not qualified");
static_assert(not std::is_constructible_v<U, fn_t<&A::k>, A &>,
              "not noexcept-qualified");
static_assert(not std::is_constructible_v<U, fn_t<&A_nice::g>, A_nice &>,
              "not const-qualified");

static_assert(not std::is_constructible_v<U, fn_t<&A_nice::h>, A_nice>,
              "cannot bind rvalue");

static_assert(not std::is_constructible_v<U, fn_t<h>, A &>,
              "not noexcept");
int h_good(A) noexcept;
static_assert(std::is_constructible_v<U, fn_t<h_good>, A &>);
